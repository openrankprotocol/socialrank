...
class DiscordChannelReader:
    ...
    def setup_events(self):
        ...
        async def on_ready():
            ...
            for guild in self.bot.guilds:
                logger.info(
                    f"  - {guild.name} (ID: {guild.id}, {len(guild.text_channels)} text channels)"
                )
    ...
    async def read_all_channels_in_server(self, guild, days_back=7, max_messages=None):
        ...
        accessible_channels = []
        logger.info(
            f"Checking access to {len(guild.text_channels)} channels in {guild.name}"
        )
        ...
        logger.info(
            f"Found {len(accessible_channels)}/{len(guild.text_channels)} accessible channels"
        )
        ...
        for i, channel in enumerate(accessible_channels, 1):
            ...
            try:
                ...
                messages = await self.read_single_channel(
                    channel, days_back, max_messages, unique_users
                )
                ...
                if not has_read_history:
                    logger.warning(
                        f"❌ #{channel.name}: Cannot read messages (no read_message_history permission) - {str(e)}"
                    )
                else:
    ...
    async def read_single_channel(
        self, channel, days_back=7, max_messages=None, unique_users=None
    ):
        """
        ...
        try:
            ...
            async for message in channel.history(after=cutoff_date, limit=max_messages):
                ...
                message_data = {
                    ...
                    "author": {
                        ...
                        if hasattr(message.author, "system")
                        else False,
                    },
                ...
                if (
                    oldest_message_timestamp is None
                    or message.created_at < oldest_message_timestamp
                ):
                ...
                if message_count % 500 == 0 and message_count > 0:
                    ...
                    logger.info(
                        f"     📊 Progress: {message_count} messages processed in {elapsed:.2f}s (avg {avg_per_msg:.3f}s/msg)"
                    )
            ...
            if message_count > 0:
                logger.info(
                    f"     ✅ Completed: {message_count} messages in {total_time:.2f}s"
                )
                if (
                    total_reaction_time > 0.5
                ):  # Only show if reactions took significant time
                    logger.info(
                        f"     ⏰ Total reaction processing time: {total_reaction_time:.2f}s"
                    )
                if oldest_message_timestamp:
                    logger.info(
                        f"     📅 Oldest message: {oldest_message_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}"
                    )
            else:
        ...
        for reaction in message.reactions:
            try:
                ...
                async for user in reaction.users():
                    users.append(
                        {"id": str(user.id), "username": user.name, "bot": user.bot}
                    )
                ...
                reactions_data.append(
                    {"emoji": str(reaction.emoji), "count": reaction.count, "users": []}
                )
    ...
    async def process_servers(self, server_ids=None, days_back=None, max_messages=None):
        ...
        logger.info(
            f"Processing {len(server_ids)} servers with {days_back} days lookback"
        )
        ...
        for i, server_id in enumerate(server_ids, 1):
            ...
            try:
                ...
                if not guild:
                    logger.error(
                        f"❌ Server {server_id} not found or bot doesn't have access"
                    )
                    continue
                ...
                if str(server_id) in overrides:
                    ...
                    logger.info(
                        f"Using server-specific settings: {server_days} days, {server_max_messages} max messages"
                    )

                # Process the server
                server_data = await self.read_all_channels_in_server(
                    guild, server_days, server_max_messages
                )
                ...
                results[server_id] = {
                    ...
                    "successful_channels": server_data["summary"][
                        "successful_channels"
                    ],
    ...
    async def start_and_process(
        self, server_ids=None, days_back=None, max_messages=None
    ):
        """
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Read Discord messages from multiple servers"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-messages", type=int, help="Override max messages per channel"
    )
    ...
    if not token:
        logger.error(
            "DISCORD_TOKEN not found in environment variables. Please set it in your .env file."
        )
        return
    ...
    results = await reader.start_and_process(
        server_ids=server_ids, days_back=args.days, max_messages=args.max_messages
    )
    ...
    if results:
        ...
        for server_id, result in results.items():
            if "error" in result:
                ...
                logger.info(
                    f"✅ {result['guild_name']}: {result['total_messages']} messages from {result['successful_channels']} channels"
                )
